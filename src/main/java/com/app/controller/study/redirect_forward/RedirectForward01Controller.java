...
public class RedirectForward01Controller {
	...
	public String re6(HttpServletRequest request) {
		...
		return "forward:re/re2";	//RequesrMapping에 해당하는 경로를 작성
	}
	...
	public String re7(HttpServletRequest request, RedirectAttributes ra) {
		...
		//redirect 시에 새로운 request로 요청되기 때문에 redirect 하면 넘어가지 않는다
		
		//return "rediret:/re/re2";		// /re/re2 경로로 redirect
		...
		//ra.addAttribute("msg", request.getParameter("msg"));	//경로에 파라미터 추가해서 연결
			//redirect 요청 주소 경로 뒤에 파라미터 추가해서 연결 >> 연결된 서버측 메소드에서도 경로에 있는 Parameter 확인 가능
		ra.addFlashAttribute("msg", request.getParameter("msg"));
			//redirect 요청 주소는 그대로 redirect된 서버측 메소드에서 연결해주는 화면에서 사용할 수 있도록 전달 (model 저장됨)
