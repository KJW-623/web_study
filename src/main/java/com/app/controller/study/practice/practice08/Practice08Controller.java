...
public class Practice08Controller {
	...
	@GetMapping("/member")
	public String memberList(@RequestParam String auth, Model model) {

		// 필터링 처리가 사전에 된다면, 뒤쪽에 if else 처리
		if (!(auth.equals("basic") || auth.equals("manager") || auth.equals("admin"))) {
			// auth== basic manager admin이 세 개가 아닌 경우
			// 파라미터가 잘못된 경우
		...
		List<Member> filterMemberList = new ArrayList<>();

		if ("basic".equals(auth)) {
			for (Member member : memberList) {
				if ("basic".equals(member.getType())) {
					filterMemberList.add(member);
				}
			}
		} else if ("manager".equals(auth)) {
			for (Member member : memberList) {
				if ("manager".equals(member.getType())) {
					filterMemberList.add(member);
				}
			}
		} else {
			filterMemberList.addAll(memberList);
		}

		model.addAttribute("memberList", filterMemberList);
		model.addAttribute("authType", auth);

		return "practice/practice08/memberList";
	}
}
